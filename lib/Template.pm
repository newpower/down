...
sub param {
  ...
  for (my $x = 0; $x <= $#_; $x += 2) {
    ...
    $options->{die_on_bad_params} and !exists($param_map->{$param});
   #mmm  and croak("HTML::Template : Attempt to set nonexistent parameter '$param' - this parameter name doesn't match any declarations in the template file : (die_on_bad_params => 1)");
