...
#print $client->responseContent();
 ...
 my $url = 'http://www.agro2b.ru/api/region/';
 if (($id_string eq '') or ($id_string == 0)) {
	my ($fol,$hhs) = &get_query($$varn{"dbh"},"SELECT * FROM `sh_downloads_zol`.`dic_region` where `flag_seach` = '0' order by `level`","hash","pr");
		$url='http://www.agro2b.ru/api/region/'.$$hhs[0]{id};
		my ($fol3,$hhs3) = &get_query($$varn{"dbh"},"UPDATE `dic_region` SET `flag_seach`='1' where `id` = '$$hhs[0]{id}'","ins","do");
 }
#else {$url=$url."$id_string";
#	my ($fol3,$hhs3) = &get_query($$varn{"dbh"},"UPDATE `dic_region` SET `flag_seach`='1' where `id` = '$id_string'","ins","do");
#	}
...
#    my @m_mas = split(/\,/,$res_html);
   	...
   	for my $col(0..$#m_mas_var)
	{
		...
		for my $col_perem(0..$#m_mas_perem)
		{
			...
			#if (($per eq 'name') or ($per eq 'socr') or ($per eq 'id')) {print "ddddddddddddddddddddddddddd"; $element_m_mas[1]="'".$element_m_mas[1]."'";}
			if (($element_m_mas[1] eq 'null') or ($element_m_mas[1] eq 'NULL')){}
			else
			{print "ddddddddddddddddddddddddddd"; $element_m_mas[1]="'".$element_m_mas[1]."'";}
		...
		#id, parent_id, name, socr, post_index, level, name_full_adres, flag_seach
		$fol = 1;
		my ($fol,$hhs) = &get_query($$varn{"dbh"},"SELECT * FROM `sh_downloads_zol`.`dic_region` where `id` = $hash_string{id}","hash","pr");
		if ($fol == 0)
		{
			my $str_per2 = "INSERT INTO `dic_region` (`id`, `parent_id`, `name`, `socr`, `post_index`, `level`) values ($hash_string{id}, $hash_string{parent_id}, $hash_string{name}, $hash_string{socr}, $hash_string{post_index}, $hash_string{level})";

			my ($fol3,$hhs3) = &get_query($$varn{"dbh"},$str_per2,"ins","do");
		print $str_per2."<br><br>";
