...
sub reader_rss_my
{
	...
	sub set_news()
	{
		...
		$hash_tags_read1{yandex_full_text}='';
		$hash_tags_read1{comments}='';
		...
		$hash_vinfo{res_html} =~ s/\<enclosure/\[enclosure/g;
	...
	#			exit;
		...
		$p1->empty_element_tags(1);  # configure its behaviour
			...
			while (my $token = $p1->get_tag("item")) 
			{
				...
				if (($hash_rss_data{language} eq '') and ($$hash_rss_sourch{lang} ne '')) {$hash_rss_data{language}=$$hash_rss_sourch{lang};}
