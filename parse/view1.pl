...
print "Content-Type: text/html; charset=utf-8\n\n";my $text;
	...
	if (length($m_mas[1]) == 0) {$m_mas[1]=$m_mas[0];}
		...
		my ($fol,$hhs) = &get_query($varinfo{dbh},"	SELECT * FROM `zol_doska_contragent` where 1 order by `id` desc limit 0,100;","hash","pr");
		#my ($fol,$hhs) = &get_query($varinfo{dbh},"	SELECT * FROM `dic_region` where `name` like '%Москва%' and 1 order by `id` desc limit 0,100;","hash","pr");
