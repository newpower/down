...
#while ($parametr > 0) {
my ($fol,$hhs) = &get_query($varinfo{dbh},"	SELECT `id`,`text`,`url` FROM t_page where `flag_downloads` =0 and `flag_use`=0 and `id` > 0 limit 0,1500;","hash","pr");
#my ($fol,$hhs) = &get_query($varinfo{dbh},"	SELECT `id`,`text`,`url` FROM cash where `id` = '51285' limit 0,1","hash","pr");
	...
	for my $col(0..$fol-1){
		...
		print "<br>$varinfo{id} -- $varinfo{url}<br>Next";
		
		$hash1{url}=$$hhs[$col]{'url'};
    	$hash1{id}=$$hhs[$col]{'id'};
		...
		$varinfo{res_html} =~ s/\'/\"/g;
			...
			my ($fol8,$hhs8) = &get_query($varinfo{dbh},"UPDATE `t_page` SET `flag_downloads`=1,`text`=\'".$varinfo{res_html}."\' where `id` = \'".$varinfo{id}."\' limit 1","ins","pr");
...
my $text;
		...
		#$p = HTML::TokeParser->new(\$tex_ish) || die print "Can't open: $!";
